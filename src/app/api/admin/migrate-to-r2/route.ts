...
import { isR2Configured, getR2Config, buildR2PublicUrl, uploadR2Object } from "@/lib/r2/client";
...
export async function GET(request: Request) {
  ...
  if (action === "diagnose") {
    ...
    if (r2Configured) {
      ...
      try {
        ...
        testUploadResult = {
          ...
          testKey,
          testUrl,
          message: "测试文件已上传到R2，请验证testUrl是否可访问",
        };
      } catch (err) {
        testUploadResult = {
          success: false,
    ...
    return Response.json({
      ...
      envVars,
      r2ConfigInfo,
      defaultR2PublicUrl: r2PublicUrl,
      testUploadResult,
      migrationStats: {
        total: totalCount ?? 0,
        alreadyR2: r2Count ?? 0,
